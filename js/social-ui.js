export function buildSocialUi(){
  if(!document.querySelector('link[href^="social.css"]')){
    const link=document.createElement('link');link.rel='stylesheet';link.href='social.css?v=20260924-pipelinefilters1';document.head.appendChild(link);
  }
  ...
  if(view){view.innerHTML=`
    <div class="social-shell">
      ...
      <section class="social-tab-panel" id="social-tab-pipeline"><div class="card social-pipeline-card"><div class="social-pipeline-toolbar"><div><div class="panel-kicker">Produzione</div><h3>Pipeline contenuti</h3><p>Segui ogni contenuto dall'idea alla pubblicazione.</p></div><div class="social-pipeline-controls"><button class="icon-btn" id="socialPipelinePrev" type="button" aria-label="Scorri pipeline a sinistra">‹</button><button class="icon-btn" id="socialPipelineNext" type="button" aria-label="Scorri pipeline a destra">›</button></div></div><div class="social-pipeline-filters"><div class="field social-pipeline-period"><label>Periodo</label><select id="socialPipelinePeriod"><option value="all">Tutti</option><option value="year">Quest'anno</option><option value="next3">Questo mese + prossimi 3 mesi</option><option value="month" selected>Questo mese</option><option value="week">Questa settimana</option></select></div><div class="field"><label>Da</label><input id="socialPipelineFrom" type="date"></div><div class="field"><label>A</label><input id="socialPipelineTo" type="date"></div><button id="socialPipelineClearDates" class="btn soft social-pipeline-clear-dates" type="button">Azzera date</button><div id="socialPipelineFilterSummary" class="social-pipeline-filter-summary"></div></div><div class="social-pipeline-wrap" id="socialPipelineWrap"><div class="social-pipeline" id="socialPipeline"></div></div></div></section>
