...
export async function loadRemote(){
  ...
  const [er,pr]=await Promise.all([db.from('events').select('*').order('event_date'),db.from('event_registrations').select('*')]);
  if(er.error||pr.error){console.error(er.error||pr.error);syncStatus('error','Errore DB');toast('Errore nel caricamento dati');return}
  app.state.events=(er.data||[]).map(mapEvent);app.state.people=(pr.data||[]).map(mapPerson);
...
export function render(){
  ...
  $('eventName').textContent=e.name;$('eventMeta').textContent=`${fmtDate(e.date)}${e.time?' · '+e.time:''}${e.place?' · '+e.place:''}${e.capacity?' · capienza '+e.capacity:''}`;
...
async function deleteEvent(id){const e=app.state.events.find(x=>x.id===id);if(!e||!confirm(`Eliminare “${e.name}” e tutte le relative iscrizioni?`))return;const {error}=await db.from('events').delete().eq('id',id);if(error)return toast(error.message);toast('Evento eliminato');await loadRemote()}
...
export function initEvents(){
  ...
  $('eventForm').addEventListener('submit',async ev=>{ev.preventDefault();const base={name:$('eName').value.trim(),event_date:$('eDate').value,event_time:$('eTime').value||null,place:$('ePlace').value.trim()||null,capacity:Number($('eCapacity').value)||0,price:$('ePrice').value===''?null:Number($('ePrice').value),notes:$('eNotes').value.trim()||null};let result;if(app.editEventId)result=await db.from('events').update({...base,updated_at:new Date().toISOString()}).eq('id',app.editEventId);else result=await db.from('events').insert({...base,created_by:app.currentUser.id});if(result.error)return toast(result.error.message);$('eventDlg').close();toast(app.editEventId?'Evento aggiornato':'Evento creato');await loadRemote()});
