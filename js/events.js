...
function mapEvent(r){return{id:r.id,name:r.name,date:r.event_date,time:r.event_time?.slice(0,5)||'',place:r.place||'',capacity:r.capacity||0,price:r.price??'',notes:r.notes||'',guestVisible:!!r.guest_visible,guestDescription:r.guest_description||''}}
...
function ensureGuestFields(){
 ...
 const notes=$('eNotes')?.closest('.field');
 if(!notes)return;
 notes.insertAdjacentHTML('afterend',`<div class="field full guest-event-settings"><label>Pagina guest / area soci</label><label class="guest-visibility-check"><input id="eGuestVisible" type="checkbox"> Visibile ai soci nella pagina guest</label><label>Descrizione per i soci</label><textarea id="eGuestDescription" rows="3" placeholder="Testo pubblico dell'evento. Le note interne sopra non verranno mai mostrate ai Guest."></textarea></div>`);
}
...
export function render(){
  ...
  $('eventName').textContent=e.name;$('eventMeta').textContent=`${fmtDate(e.date)}${e.time?' · '+e.time:''}${e.place?' · '+e.place:''}${e.capacity?' · capienza '+e.capacity:''}${collaborators?' · '+collaborators+' collaboratori':''}${e.guestVisible?' · visibile ai soci':''}`;
...
export function initEvents(){
  ...
  $('eventForm').addEventListener('submit',async ev=>{ev.preventDefault();const base={name:$('eName').value.trim(),event_date:$('eDate').value,event_time:$('eTime').value||null,place:$('ePlace').value.trim()||null,capacity:Number($('eCapacity').value)||0,price:$('ePrice').value===''?null:Number($('ePrice').value),notes:$('eNotes').value.trim()||null,guest_visible:$('eGuestVisible').checked,guest_description:$('eGuestDescription').value.trim()||null};let result;if(app.editEventId)result=await db.from('events').update({...base,updated_at:new Date().toISOString()}).eq('id',app.editEventId).select('id').single();else result=await db.from('events').insert({...base,created_by:app.currentUser.id}).select('id').single();if(result.error)return toast(result.error.message);try{await syncEventContacts(result.data.id,base)}catch(error){console.error(error);return toast('Evento salvato, ma errore nel collegamento collaboratori')}$('eventDlg').close();toast(app.editEventId?'Evento aggiornato':'Evento creato');await loadRemote()});
