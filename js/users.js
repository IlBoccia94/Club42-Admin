...
function ensurePermissionGuide(){
  ...
  if(!$('roleInfoDlg'))document.body.insertAdjacentHTML('beforeend',`<dialog id="roleInfoDlg"><div class="modal"><div class="modal-head"><div><div class="panel-kicker">Accessi gestionale</div><h3>Visibilità dei ruoli</h3></div><button type="button" class="close" id="roleInfoClose">×</button></div><div class="role-info-grid">
    <article class="role-info-card"><h4>Admin</h4><p><strong>Accesso completo.</strong> Gestisce Eventi, Soci, Cassa, Progetti, Task, Social, Contatti e Utenti. È l'unico ruolo che può amministrare account e ruoli, accedere al registro Soci completo, alle eliminazioni massive e alle impostazioni strutturali.</p></article>
    <article class="role-info-card"><h4>Tesoriere</h4><p>Può vedere Dashboard ed Eventi in sola lettura, consultare i <strong>dati essenziali dei Soci</strong>, gestire completamente la <strong>Cassa</strong>, lavorare su Progetti, Task e Social e <strong>gestire completamente Contatti e collaborazioni</strong>. Non vede Utenti.</p></article>
    <article class="role-info-card"><h4>Staff</h4><p>Può gestire Eventi e partecipanti, consultare i <strong>dati essenziali dei Soci</strong>, vedere la <strong>Cassa in sola lettura</strong>, lavorare su Progetti, Task e Social e <strong>gestire completamente Contatti e collaborazioni</strong>. Non vede Utenti.</p></article>
  ...
  </div><div class="role-info-note">La sezione <strong>Contatti</strong> è operativa per Admin, Staff e Tesoriere: possono consultare e modificare schede, recapiti, note, tag, preferiti e storico collaborazioni. La sezione <strong>Utenti</strong> resta esclusivamente Admin.</div><div class="modal-actions"><button type="button" class="btn primary" id="roleInfoOk">Ho capito</button></div></div></dialog>`);
}
...
function renderUsers(){$('usersActive').textContent=app.adminUsers.filter(u=>u.status==='active').length;$('usersPending').textContent=app.adminUsers.filter(u=>u.status==='pending').length;$('usersDisabled').textContent=app.adminUsers.filter(u=>u.status==='disabled').length;const rows=app.adminUsers.map(u=>`<tr><td><b>${esc(u.display_name||'Senza nome')}</b><br><span class="muted">${esc(u.email||'')}</span></td><td><span class="role-pill">${esc(roleLabel(u.role))}</span></td><td><span class="status-pill status-${u.status}">${statusLabel(u.status)}</span></td><td>${fmtDate((u.created_at||'').slice(0,10))}</td><td>${userActions(u)}</td></tr>`).join('');$('usersTableBody').innerHTML=rows||'<tr><td colspan="5" class="empty">Nessun utente.</td></tr>';$('usersCards').innerHTML=app.adminUsers.map(u=>`<div class="user-card"><h4>${esc(u.display_name||'Senza nome')}</h4><div class="email">${esc(u.email||'')}</div><div class="meta"><span class="role-pill">${esc(roleLabel(u.role))}</span><span class="status-pill status-${u.status}">${statusLabel(u.status)}</span></div>${userActions(u)}</div>`).join('')||'<div class="empty">Nessun utente.</div>'}
function openUser(id){const u=app.adminUsers.find(x=>x.user_id===id);if(!u)return;ensureRoleOptions();app.editUserId=id;$('uName').value=u.display_name||'';$('uEmail').value=u.email||'';$('uRole').value=u.role;$('uStatus').value=u.status;$('userDlg').showModal()}
async function quickUserStatus(id,status){const u=app.adminUsers.find(x=>x.user_id===id);if(!u)return;try{await adminApi('update',{userId:id,displayName:u.display_name,role:u.role,status});toast(status==='active'?'Utente attivato':'Utente disabilitato');await loadUsers()}catch(e){toast(e.message)}}
...
export function initUsers(){
  ...
  $('inviteUserBtn').onclick=()=>{ensureRoleOptions();$('iName').value='';$('iEmail').value='';$('iRole').value='staff';$('inviteDlg').showModal()};
